...
class Zombie:
    ...
    def set_random_location(self):
        self.tx, self.ty = random.randint(100, 1200 - 100), random.randint(100, 720 - 100)
        return BehaviorTree.SUCCESS
    ...
    # BALL COUNT 비교
    def compare_ball_count_boy(self):   #boy ball이 더 큼
        if self.ball_count <= play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    def compare_ball_count_zombie(self):    #zombie ball 이 더 큼
        if self.ball_count  > play_mode.boy.ball_count:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def run_from_boy(self, r=0.5):
        ...
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            ...
            return BehaviorTree.RUNNING
    def run_slightly_to(self, tx, ty):
        self.dir = math.atan2(ty - self.y, tx - self.x)
        self.speed = RUN_SPEED_PPS
        self.x -= self.speed * math.cos(self.dir) * game_framework.frame_time
        self.y -= self.speed * math.sin(self.dir) * game_framework.frame_time
    ...
    def build_behavior_tree(self):
        ...
        a3 = Action('Set randint location', self.set_random_location)
        a4 = Action('소년 한테 접근', self.move_to_boy)
        a5 = Action ('순찰 위치 가져오기', self.get_patrol_location)
        a6 = Action ('소년 한테 도망', self.run_from_boy)



        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        # 볼카운트 비교
        c2 = Condition('Ball count 비교 boy가 더 큼', self.compare_ball_count_boy)

        c3 = Condition('Ball count 비교 zombie가 더 큼', self.compare_ball_count_zombie)

        SEQ_move_to_target_location = Sequence('Move to target location', a1, a2)

        SEQ_wander = Sequence('Wander', a3, a2)
        SEQ_compare_boy = Sequence('소년의 공이 더 많은지 판단', c1, c2)
        SEQ_compare_zombie = Sequence('좀비의 공이 더 많은지 판단', c1, c3)

        SEQ_chase_boy = Sequence('소년을 추적', SEQ_compare_boy, a4)

        SEQ_run_zombie = Sequence('소년을 추적', SEQ_compare_zombie, a6)
        ...
        root = SEL_chase_or_wander = Selector('추적 또는 도망 또는 배회', SEQ_chase_boy, SEQ_run_zombie, SEQ_wander)
